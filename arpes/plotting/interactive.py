...
def make_bokeh_tool(arr: Union[xr.DataArray, str], notebook_url='localhost:8888',
    ...
    if len({'kx', 'ky', 'kz', 'kp'}.intersection(set(arr.dims))) > 0:
    ...
    handler = FunctionHandler(bokeh_tool(arr, **kwargs))
...
def bokeh_tool(arr: xr.DataArray, app_main_size=600, app_marginal_size=300):
    ...
    def bokeh_tool_handler(doc):
        ...
        def prep_image(image_arr):
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                if app_context['color_mode'] == 'linear':
        ...
        figures, plots, app_widgets = app_context['figures'], app_context['plots'], app_context['widgets']
        app_context['cursor'] = [np.mean(app_context['data_range']['x']),
                                 np.mean(app_context['data_range']['y']),
                                 np.mean(app_context['data_range']['z'])]  # Try a sensible default

        # create the main inset plot
        main_image = arr.sel(**dict([[arr.dims[2], app_context['cursor'][2]]]), method='nearest')
        prepped_main_image = prep_image(main_image)
        app_context['color_maps']['main'] = LinearColorMapper(
        ...
        z_marginal_data = arr.sel(**dict([[arr.dims[0], app_context['cursor'][0]], [arr.dims[1], app_context['cursor'][1]]]), method='nearest')
        ...
        figures['z_marginal'] = figure(
            plot_width=app_marginal_size, plot_height=app_marginal_size,
            x_range=app_context['data_range']['z'], y_range=(np.min(z_marginal_data.values), np.max(z_marginal_data.values)),
        ...
        app_widgets['info_div'] = Div(text='', width=app_marginal_size, height=100)
        ...
        if arr.dims[2] == 'eV':
            ...
            app_context['z_model'] = GStepBModel
            ...
            plots['t0_marker'] = figures['z_marginal'].line(
                x=[float(arr.attrs.get('t0')), float(arr.attrs.get('t0'))], y=[0, 1000000], line_color='black', line_dash='dashed')
            t0 = float(arr.attrs['t0'])
            app_context['z_model'] = ExponentialDecayCModel
            try:
        ...
        horiz_cursor_x = list(app_context['data_range']['x'])
        horiz_cursor_y = [0, 0]
        vert_cursor_x = [0, 0]
        vert_cursor_y = list(app_context['data_range']['y'])

        # Create the bottom marginal plot
        bottom_image = arr.sel(**dict([[arr.dims[1], app_context['cursor'][1]]]), method='nearest')
        prepped_bottom_image = prep_image(bottom_image)
        app_context['color_maps']['bottom'] = LinearColorMapper(
            default_palette, low=np.min(prepped_bottom_image), high=np.max(prepped_bottom_image), nan_color='black')
        figures['bottom'] = figure(plot_width=app_main_size, plot_height=app_marginal_size,
                             title=None, x_range=figures['main'].x_range, y_range=figures['z_marginal'].x_range,
                             x_axis_location='above',
                             toolbar_location=None, tools=[])
        figures['bottom'].xaxis.major_label_text_font_size = '0pt'
        plots['bottom'] = figures['bottom'].image([prepped_bottom_image.T], x=app_context['data_range']['x'][0], y=app_context['data_range']['z'][0],
                                         dw=app_context['data_range']['x'][1] - app_context['data_range']['x'][0],
                                         dh=app_context['data_range']['z'][1] - app_context['data_range']['z'][0],
                                         color_mapper=app_context['color_maps']['bottom'])
        bottom_marginal = bottom_image.sel(**dict([[arr.dims[2], app_context['cursor'][2]]]), method='nearest')
        figures['bottom_marginal'] = figure(plot_width=app_main_size, plot_height=200,
        ...
        right_image = arr.sel(**dict([[arr.dims[0], app_context['cursor'][0]]]), method='nearest')
        prepped_right_image = prep_image(right_image)
        app_context['color_maps']['right'] = LinearColorMapper(
            default_palette, low=np.min(prepped_right_image), high=np.max(prepped_right_image), nan_color='black')
        figures['right'] = figure(plot_width=app_marginal_size, plot_height=app_main_size, title=None,
                                  x_range=figures['z_marginal'].x_range, y_range=figures['main'].y_range,
                                  toolbar_location=None, tools=[])
        figures['right'].yaxis.major_label_text_font_size = '0pt'
        plots['right'] = figures['right'].image([prepped_right_image], x=app_context['data_range']['z'][0], y=app_context['data_range']['y'][0],
                                       dw=app_context['data_range']['z'][1] - app_context['data_range']['z'][0],
                                       dh=app_context['data_range']['y'][1] - app_context['data_range']['y'][0],
                                       color_mapper=app_context['color_maps']['right'])
        right_marginal = right_image.sel(**dict([[arr.dims[2], app_context['cursor'][2]]]), method='nearest')
        figures['right_marginal'] = figure(plot_width=200, plot_height=app_main_size,
        ...
        def update_cursor(vert_x, horiz_y):
            horiz_y[0] = horiz_y[1] = app_context['cursor'][1]
            vert_x[0] = vert_x[1] = app_context['cursor'][0]

        update_cursor(vert_cursor_x, horiz_cursor_y)

        cursor_lines = figures['main'].multi_line(xs=[horiz_cursor_x, vert_cursor_x], ys=[horiz_cursor_y, vert_cursor_y],
        ...
        def set_show_stat_variation(should_show):
            ...
            app_context['show_stat_variation'] = should_show

            if should_show:
                main_image_data = arr.sel(**dict([[arr.dims[2], cursor[2]]]), method='nearest')
                update_stat_variation('z', arr.sel(**dict([[arr.dims[0], cursor[0]], [arr.dims[1], cursor[1]]]), method='nearest'))
                update_stat_variation('bottom', main_image_data.sel(**dict([[arr.dims[1], cursor[1]]]), method='nearest'))
                update_stat_variation('right', main_image_data.sel(**dict([[arr.dims[0], cursor[0]]]), method='nearest'))
        ...
        def on_change_color_mode(attr, old, new_color_mode):
            app_context['color_mode'] = new_color_mode
            if old is None or old != new_color_mode:
                ...
                main_image_data = arr.sel(**dict([[arr.dims[2], cursor[2]]]), method='nearest')
                prepped_right_image = prep_image(right_image_data)
                prepped_bottom_image = prep_image(bottom_image_data)
                prepped_main_image = prep_image(main_image_data)
        ...
        def place_symmetry_point():
            cursor_dict = dict(zip(arr.dims, app_context['cursor']))
        ...
        layout = row(column(figures['main'], figures['bottom'], figures['bottom_marginal']),
                     column(figures['right'], figures['z_marginal'], Spacer(width=app_marginal_size, height=200)),
                     column(figures['right_marginal'], Spacer(width=200, height=app_marginal_size), Spacer(width=200, height=200)),
                     column(widgetbox(widgets.Dropdown(label='Pointer Mode', button_type='primary', menu=POINTER_MODES)),
                            widgets.Tabs(tabs=[
                                ...
                                widgets.Panel(child=widgetbox(
                                    ...
                                    widgets.DataTable(source=scan_info_source, columns=scan_info_columns, width=400, height=400),
        ...
        def update_colormap_for(plot_name):
            def update_plot_colormap(attr, old, new):
                ...
                app_context['color_maps'][plot_name].update(low=low + new[0] / 100 * dynamic_range,
        ...
        def click_z_marginal(event):
            ...
            if app_context['show_stat_variation']:
