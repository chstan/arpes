...
def gaussian_convolve(model_instance):
    ...
    :return:
    """
    return XConvolutionCompositeModel(model_instance, GaussianModel(prefix="conv_"), convolve)
...
def twolorentzian_gstep(
    x, gamma, t_gamma, center, t_center, amp, t_amp, lin_bkg, const_bkg, g_center, sigma, erf_amp
):
...
def affine_broadened_fd(
    ...
    return (
        gaussian_filter((const_bkg + lin_bkg * dx) * fermi, sigma=conv_width / x_scaling) + offset
    )
...
class SplitLorentzianModel(XModelMixin, lf.models.SplitLorentzianModel):
    def _set_paramhints_prefix(self):
        ...
        self.set_param_hint("fwhm", expr="{prefix}sigma+{prefix}sigma_r".format(prefix=self.prefix))
        self.set_param_hint(
            "height",
            expr="2*{prefix}amplitude/{:.7f}/max({}, ({prefix}sigma+{prefix}sigma_r))".format(
