...
def load_scan(scan_desc, **kwargs):
    ...
    if load_fn is not None:
        return load_fn(scan_desc, **kwargs)
...
def load_MC(metadata: dict=None, filename: str=None, **kwargs):
    """
...
def load_SES(metadata: dict=None, filename: str=None, robust_dimension_labels=False, **kwargs):
    """
...
def load_SToF(metadata: dict=None, filename: str=None, **kwargs):
    if metadata is None and filename is None:
        warnings.warn('Attempting to make due without user associated metadata for the file')
        raise TypeError('Expected a dictionary of metadata with the location of the file')
