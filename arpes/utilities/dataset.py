...
def rename_files(dry=True, path=None, extensions=None, starting_index=1):
    ...
    for i, file in enumerate(files):
        ...
        short_output = shorten_left(
            str(file.parent / (file.stem + extra + file.suffix)), max_length=20 + len(extra)
        )
...
def infer_data_path(
    file: int, scan_desc: pd.Series, allow_soft_match: bool = False, use_regex: bool = True
) -> str:
...
def default_dataset(
    workspace: Optional[Any] = None, match: Optional[str] = None, **kwargs: Any
) -> pd.DataFrame:
...
def with_inferred_columns(df: pd.DataFrame):
    ...
    if "spectrum_type" not in df.columns:
        warnings.warn(
            "You need to have a spectrum_type column. Try using `prepare_raw_files` first."
        )
        return df
...
def safe_read(path: str, **kwargs: Any) -> pd.DataFrame:
    ...
    warnings.warn(
        "You must supply both a `file` and a `location` column in your "
...
def modern_clean_xlsx_dataset(
    path, allow_soft_match=False, with_inferred_cols=True, write=False, **kwargs
):
    ...
    for index, row in joined.iterrows():
        ...
        for key, value in row.iteritems():
            ...
            if key == "id" and is_blank(row.id):
                ...
                and is_blank(value)
                and not is_blank(joined.loc[last_index, (key,)])
            ):
...
def clean_xlsx_dataset(
    path: str,
    allow_soft_match: bool = False,
    write: bool = True,
    with_inferred_cols: bool = True,
    warn_on_exists: bool = False,
    **kwargs: Any
) -> pd.DataFrame:
    ...
    for index, row in ds.sort_index().iterrows():
        ...
        for key, value in row.iteritems():
            ...
            if key == "path":
                ...
                last_index is not None
                and is_blank(value)
                and not is_blank(ds.loc[last_index, (key,)])
            ):
